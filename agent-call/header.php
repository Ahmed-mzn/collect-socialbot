...
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true || $_SESSION["permission"] != 'agent-call'){
    header("location: /");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['set_store']) && !empty($_POST['set_store'])){
	// echo 'changed store';

	$ch = curl_init("http://localhost/khaled/api/get-user-info.php?user_id=".$_POST['set_store']);
...
// get all clients
$sth = $connect->prepare("SELECT * FROM CLIENTS WHERE user_id!=".intval($_SESSION['store_user_id']));
$sth->execute();
...
<html lang="en">
	...
	<body class=" ltr main-body app sidebar-mini">
		...
		<div class="page">
					...
					<aside class="app-sidebar">
						...
						<div class="main-sidemenu">
							<div class="slide-left disabled" id="slide-left"><svg xmlns="http://www.w3.org/2000/svg" fill="#7b8191" width="24" height="24" viewBox="0 0 24 24"><path d="M13.293 6.293 7.586 12l5.707 5.707 1.414-1.414L10.414 12l4.293-4.293z"/></svg></div>
								...
								<div class="container">
									<div class="form-group">
										<label class="form-label">المتجر الحالي</label>
										<select onchange="storeChange(this)" name="country" class="form-control form-select" data-bs-placeholder="Select Country">
											<option value="<?= $_SESSION['store_user_id'] ?>"><?= $_SESSION['store_website'] ?></option>
											<?php foreach($clients as $client){?>
												<option value="<?= $client['user_id'] ?>"><?= $client['website'] ?></option>
											<?php } ?>
										</select>
									</div>
								</div>
							<ul class="side-menu">
								...
								<li class="side-item side-item-category"> مركز المساعدة</li>
								<li class="slide">
									<a class="side-menu__item" href="orders.php"><svg xmlns="http://www.w3.org/2000/svg"  class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M7 18c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zM1 2v2h2l3.6 7.59-1.35 2.45c-.16.28-.25.61-.25.96 0 1.1.9 2 2 2h12v-2H7.42c-.14 0-.25-.11-.25-.25l.03-.12.9-1.63h7.45c.75 0 1.41-.41 1.75-1.03l3.58-6.49c.08-.14.12-.31.12-.48 0-.55-.45-1-1-1H5.21l-.94-2H1zm16 16c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2z"/></svg><span class="side-menu__label">طلبات المتجر</span></a>
								</li>
							</ul>
			...
			<form id="setStoreForm" class="d-none" action="" method="post">
				<input id="set_store" type="text" class="d-none" name="set_store">
			</form>
...
<script>
	function storeChange(el){
		// console.log(el.value);
		document.getElementById('set_store').value = el.value;
		document.getElementById('setStoreForm').submit();
	}
</script>
<?php
// echo '<br/><br/><br/>';
